...
5. **Implement** — use `editFiles`. Pick the rules block for your track:
   ...
   **Python files MUST:**
   ...
   - **Agent Identity RBAC (LAB 3).** A 401 from the memory backend calling the embedding model means the project uses a **separate ServiceIdentity SP** for data-plane calls. Read it from `properties.agentIdentity.agentIdentityId` on the project resource (`az resource show --ids .../projects/<project> --api-version 2025-04-01-preview`) and grant **`Cognitive Services OpenAI User`** on both the account and the project, plus **`Cognitive Services User`** on the account, to THAT SP — NOT the account MI or the project subresource MI.
