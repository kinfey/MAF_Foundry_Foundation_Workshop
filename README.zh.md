# ZavaShop 供应链智能化 Workshop

> 用 **Microsoft Agent Framework + Microsoft Foundry (GPT-5.5)**，由 **GitHub Copilot 内置 SKILL** 协助，从单 Agent 一路构建到多 Agent Workflow + AG-UI 前端的供应链智能体系统。
...
> 所有 LAB 的入口都是 **`@zavashop-coding-agent`**（定义在 [.github/agents/zavashop-coding-agent.agent.md](.github/agents/zavashop-coding-agent.agent.md)）—— 它会按下表自动 `read_file` 加载对应的 SKILL.md，然后帮你完成 LAB 任务。

| LAB | 故事场景 | 核心能力 | 由哪个 SKILL 协助 |
|-----|---------|---------|-------------------|
| **LAB 1** | 仓储助手 Zara | 单 Agent + Function Tools + MCP | [`agent-framework-azure-ai-py`](.github/skills/agent-framework-azure-ai-py/SKILL.md) |
| **LAB 2** | 采购代理 Pierre | Foundry Toolbox + Agent Skills + Thread | [`agent-framework-azure-ai-py`](.github/skills/agent-framework-azure-ai-py/SKILL.md) |
| **LAB 3** | 客服 Aria | Foundry Memory + Evaluation + Red-Team | [`agent-framework-azure-ai-py`](.github/skills/agent-framework-azure-ai-py/SKILL.md) |
| **LAB 4** | 订单履约调度 | 多 Agent Workflow + HITL + Checkpoint | [`agent-framework-workflows-py`](.github/skills/agent-framework-workflows-py/SKILL.md) |
| **LAB 5** | 供应链指挥中心 | AG-UI 协议 + 共享状态 + 生成式 UI | [`agent-framework-agui-py`](.github/skills/agent-framework-agui-py/SKILL.md) |
...
| `zava_data.py` | 所有 LAB | 加载模块（`find_stock` / `find_po` / `find_supplier` / `find_contract` / `find_customer` / `find_order` / `load_*`） |
...
本 Workshop **不要用 `@workspace`**。我们在 [`.github/agents/zavashop-coding-agent.agent.md`](.github/agents/zavashop-coding-agent.agent.md) 里已经定义好了一个**专属 GitHub Copilot Coding Agent**，它知道每个 LAB 对应哪个 SKILL、按 7 步循环（Locate → Load skill → Load lab → Plan → Implement → Validate → Map to acceptance）干活。

每个 LAB 的节奏统一为：

1. 打开 `LABxx/README.md`，浏览一遍故事 + 验收标准。
2. 在 Copilot Chat 里输入（**Agent 模式**）：
   ```
   @zavashop-coding-agent I'm doing LAB X — <你这一关的目标，比如 "build the inventory agent Zara">
   ```
3. Coding Agent 会自动：
   - `read_file` 加载 `.github/skills/<对应-skill>/SKILL.md`
   - `read_file` 加载本 LAB 的 README
...
│   └── skills/
│       ├── agent-framework-azure-ai-py/     # LAB 1 / 2 / 3
